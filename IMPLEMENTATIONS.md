...
- [x] **Story:** As the product team, we want the Flutter app wired to live APIs so we can validate the full experience.
  ...
  - **Verification Steps:**
    ...
    13. After pulling these changes, run `flutter pub get` within `sankofa_app/` so the updated dependency tree (including `flutter_contact_picker` for the invite workflow and `image_picker` for KYC) is ready before compiling the mobile or web client.
