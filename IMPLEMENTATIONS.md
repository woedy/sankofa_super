...
- [ ] **Story:** As the product team, we want the Flutter app wired to live APIs so we can validate the full experience.
  ...
  - **Verification Steps:**
    ...
    2. Launch the Flutter app with dart-defines that point to the backend, e.g.:
       ```bash
       flutter run \
         --dart-define=SANKOFA_ENV=local \
         --dart-define=API_BASE_URL=http://10.0.2.2:8000
       ```
       (Use `http://127.0.0.1:8000` when running on web.)
    3. From the login screen, enter a real phone number registered in the backend seed data, request an OTP, and confirm the SMS/console code. After verification the app should navigate to the main dashboard (or KYC screen if the user is not verified).
