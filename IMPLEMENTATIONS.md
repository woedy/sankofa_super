...
- [ ] **Story:** As the product team, we want the Flutter app wired to live APIs so we can validate the full experience.
  ...
  - **Verification Steps:**
    ...
    9. After pulling these changes, run `flutter pub get` within `sankofa_app/` so the new `image_picker` plugin is available before compiling the mobile or web client.
