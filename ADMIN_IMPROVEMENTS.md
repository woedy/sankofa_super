...
### [x] P1 — Public Group Creation Wizard _(Completed 2025-01-26)_
**User Story**: As a platform operator, I need to launch a public susu cohort directly from the admin so that qualified members can discover and join it from the mobile app without engineering support.

**Acceptance Criteria**
- An "Add public group" call-to-action is visible above the lifecycle filters on the Groups page and opens a multi-step wizard.
- Step 1 captures basics (name, purpose, organizer contact, location) with destructive validation when fields are missing.
- Step 2 records contribution plan details (member target, amount, frequency, collection window, start and payout dates, invite prospects) with guardrails for invalid numbers and empty dates.
- Step 3 presents a review summary, and submitting the wizard adds a new public group card with an Onboarding stage, invite stats, and the detail drawer opened for immediate follow-up.

**Completion Notes**
- Added a reusable wizard with step indicators, contextual guidance, and validation copy that mirrors the mobile onboarding expectations.
- Pushed the new cohort into the lifecycle grid with automatic filter alignment (public visibility + onboarding stage) and optimistic toast confirmation for operators.
- Seeded invite, reminder, and organizer metadata so the refreshed detail drawer—including the public join flow panel—reflects the operator’s inputs instantly.

**Test Checklist**
- ✅ Click **Add public group**, leave a required field empty, and confirm the wizard surfaces destructive validation preventing progress.
- ✅ Complete all steps with realistic data and ensure the newly created cohort appears in the grid with the Onboarding badge, correct invite stats, and opens automatically.
- ✅ In the detail drawer, review the invite pipeline and public join flow panel to verify they display the wizard inputs and remain scrollable/responsive in both themes.
