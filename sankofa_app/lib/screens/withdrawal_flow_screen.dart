...
class _WithdrawalFlowScreenState extends State<WithdrawalFlowScreen> {
  ...
  Future<void> _submitWithdrawal() async {
    ...
    if (status == WithdrawalSubmissionStatus.success) {
      final newBalance = (user.walletBalance - amount).clamp(0, double.infinity);
  ...
  Future<void> _showOutcomeSheet({
    ...
    final message = switch (status) {
      WithdrawalSubmissionStatus.success =>
          'Expect a confirmation once the funds land in your ${destination.name}.',
      WithdrawalSubmissionStatus.pending =>
          'Compliance needs a quick look at this cash-out. We\'ll notify you once it clears.',
      WithdrawalSubmissionStatus.failed =>
          'Support will reach out shortly. You can try again after updating your compliance details.',
    };
