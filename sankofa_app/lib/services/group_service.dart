...
class GroupService {
  ...
  Future<SusuGroupModel> createGroupFromDraft(GroupDraftModel draft) async {
    ...
    for (var i = 0; i < draft.memberNames.length; i++) {
      invites.add({
        'name': draft.memberNames[i],
        'phoneNumber': _generateMockPhone(now.millisecondsSinceEpoch + i),
      });
    }
  ...
  String _generateMockPhone(int seed) {
    final normalized = (seed.abs() % 10000000).toString().padLeft(7, '0');
    final prefix = '20';
    return '+233$prefix$normalized';
  }
}
